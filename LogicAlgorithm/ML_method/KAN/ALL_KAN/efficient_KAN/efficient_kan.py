...
# -*- coding: utf-8 -*-
"""
function description: 此文件是Github上Blealtan于20240602日版本的efficient-kan项目下的src/efficient_kan/efficient_kan.py
author: Blealtan
contact: https://github.com/Blealtan/efficient-kan/tree/master/src/efficient_kan
...
class KANLinear(torch.nn.Module):
    def __init__(self, in_features, out_features, grid_size=5, spline_order=3, scale_noise=0.1,
        ...
        # 在保存模型时将缓冲区的状态也保存下来，并能够在模型迁移或者调整时重新初始化缓冲区
        # register_buffer(name, tensor),name是缓冲区的名字，tensor是要注册的缓冲区。
        self.register_buffer("grid", grid)  # tensor([-3,-2,-1,0,1,2,3,4,5,6,7,8]) * (2/5) + (-1))
    ...
    def reset_parameters(self):
        ...
        with torch.no_grad():
            ...
            # 样条函数的参数初始化
            self.spline_weight.data.copy_((self.scale_spline
                                           if not self.enable_standalone_scale_spline else 1.0)
                                          * self.curve2coeff(self.grid.T[self.spline_order:
                                                                         -self.spline_order], noise, ))
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.view(-1, self.in_features)  # (batch, input_size)
        ...
        spline_output = F.linear(self.b_splines(x).view(x.size(0), -1),
                                 # self.b_splines(x): (batch, input_size, batch/input_size)——(batch, batch)
                                 self.scaled_spline_weight.view(self.out_features, -1)
                                 # self.scaled_spline_weight: (output_size, input_size, batch/input_size)
                                 # ——(output_size, batch)
                                 )
        # (batch, batch) + (output_size, batch) / (batch, output_size)
    ...
    def update_grid(self, x: torch.Tensor, margin=0.01):
        ...
        grid_adaptive = x_sorted[torch.linspace(0, batch - 1, self.grid_size + 1, dtype=torch.int64,
                                                device=x.device)]

        uniform_step = (x_sorted[-1] - x_sorted[0] + 2 * margin) / self.grid_size
        grid_uniform = (torch.arange(self.grid_size + 1, dtype=torch.float32,
                                     device=x.device).unsqueeze(1) * uniform_step + x_sorted[0] - margin)

        grid = self.grid_eps * grid_uniform + (1 - self.grid_eps) * grid_adaptive
        grid = torch.concatenate([grid[:1] - uniform_step
                                  * torch.arange(self.spline_order, 0, -1, device=x.device).unsqueeze(1),
                                  grid, grid[-1:] + uniform_step
...
class KAN(torch.nn.Module):
    def __init__(
        ...
        for in_features, out_features in zip(layers_hidden, layers_hidden[1:]):
            self.layers.append(KANLinear(in_features, out_features, grid_size=grid_size,
        ...
        # ModuleList((0-1): 2 x KANLinear((base_activation): Identity()))
    ...
    """
    description: 创建返回KAN模型函数，方便后续画图及输出公式等
    author: TangKan
    contact: 785455964@qq.com
    IDE: PyCharm Community Edition 2021.2.3
    time: 2024/6/26 11:35
    version: V1.1
    """
    def return_KAN_layers(self):
        return self.layers
